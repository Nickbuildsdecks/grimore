...
window.toggleAuthForm = function(isRegister) {
...
window.handleRegister = async function(event) {
  ...
  if (!usernameInput || !nicknameInput || !emailInput || !passwordInput) return;

  const username = usernameInput.value.trim();
  const storeNickname = nicknameInput.value.trim();
  const email = emailInput.value.trim();
  const password = passwordInput.value;

  if (!username || !storeNickname || !email || !password) {
  ...
  try {
    const res = await fetch('/api/auth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username, storeNickname, email, password })
    });
    const data = await res.json();
    if (data.success) {
      alert("Registration successful! You can now log in.");
      window.toggleAuthForm(false);
    ...
    alert("Network error during registration: " + e.message);
  }
};
...
function initGoogleSignInButtons() {
  ...
  const renderFallbackBtn = (container, text) => {
    if (!container) return;
    container.innerHTML = `
      <button type="button" class="btn google-signin-btn" onclick="triggerGoogleSignIn()" style="width: 100%; height: 42px; display: flex; align-items: center; justify-content: center; gap: 0.75rem; background: #ffffff; color: #1f2937; border: 1px solid #e5e7eb; font-family: 'Outfit', sans-serif; font-weight: 600; font-size: 0.88rem; border-radius: var(--radius-sm); cursor: pointer; transition: all 0.2s ease; box-shadow: 0 2px 6px rgba(0,0,0,0.15); margin: 0;">
    ...
    try {
      ...
      if (widget) {
        if (data.featuredDeck) {
          ...
          if (document.getElementById('featured-deck-points')) document.getElementById('featured-deck-points').textContent = data.featuredDeck.total_points || 0;
          if (document.getElementById('featured-deck-wins')) document.getElementById('featured-deck-wins').textContent = data.featuredDeck.total_wins || 0;
          if (document.getElementById('featured-deck-matches')) document.getElementById('featured-deck-matches').textContent = data.featuredDeck.total_matches || 0;
