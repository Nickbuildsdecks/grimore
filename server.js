...
app.get('/api/cards/versions', async (req, res) => {
  ...
  try {
    let result;
    try {
      const searchUrl = `https://api.scryfall.com/cards/search?q=!%22${encodeURIComponent(trimmedName)}%22&unique=prints`;
    ...
    let prints = (result.data || []).filter(card => {
      ...
      return Object.values(legalities).some(status => status === "legal" || status === "restricted");
    }).flatMap(card => {
      const pricesObj = card.prices || {};
      const price = getLowestUsdPrice(pricesObj);
